...
void menuLista(Node *lista){
    ...
    switch(op){
        case 1:
            insereInicio(&lista,criaNoDados());
        break;
        case 2:
            insereFim(&lista,criaNoDados());
        break;
        case 3:
            printf("\nInforme a posicao do novo Elemento: ");
            scanf("%d",&pos);
            inserePosicao(&lista,criaNoDados(),pos);
        break;
        case 4:
            printf("\nApaga o primeiro Elemento:");
            aux = removeInicio(&lista);
            aux!=NULL?printf("\n O valor %d será deletado:",aux->info.valor):printf("\nSem remoção!");
            free(aux);            
        break;
        case 5:
            printf("\nApaga o Último Elemento:");
            aux = removeFim(&lista);
            printf("\n O valor %d será deletado:",aux!=NULL?aux->info.valor:0);
            free(aux);
        break;
        case 6:
            printf("\nInforme o novo Elemento e a posição para inserir o novo Node na Lista:");
            scanf("%d",&valor);
            aux = removePosicao(&lista,valor);
        break;
